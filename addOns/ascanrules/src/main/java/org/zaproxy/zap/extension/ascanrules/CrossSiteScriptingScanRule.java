...
public class CrossSiteScriptingScanRule extends AbstractAppParamPlugin
    ...
    private List<HtmlContext> performAttack(XssAttackParam xssParam) {
        if (isStop()) {
            return null;
        }

        HttpMessage msg2 = xssParam.getMsg().cloneRequest();
        setParameter(msg2, xssParam.getParam(), xssParam.getAttack());
        try {
            sendAndReceive(msg2);
        ...
        if (xssParam.isNullByteSpecialHandling()) {
             ...
             */
            String attack = xssParam.getAttack().replaceFirst(NULL_BYTE_CHARACTER, "");
            xssParam.setAttack(attack);
            xssParam.setEvidence(attack);
        }
        ...
        if (Plugin.AlertThreshold.LOW.equals(this.getAlertThreshold())) {
            ...
            contexts =
                    hca.getHtmlContexts(
                            xssParam.isFindDecoded()
                                    ? getURLDecode(xssParam.getEvidence())
                                    : xssParam.getEvidence(),
                            null,
                            0,
                            xssParam.ignoreSafeParents);
        } else {
            ...
            contexts =
                    hca.getHtmlContexts(
                            xssParam.isFindDecoded()
                                    ? getURLDecode(xssParam.getEvidence())
                                    : xssParam.getEvidence(),
                            xssParam.getTargetContext(),
                            xssParam.getIgnoreFlags(),
                            xssParam.isIgnoreSafeParents());
        }
        if (xssParam.isMutateAttack() || !contexts.isEmpty()) {
            return contexts;
        }
        xssParam.setMsg(msg2);
        return this.mutateAttack(xssParam);
    }
     ...
     * work instead.
     */
    private List<HtmlContext> mutateAttack(XssAttackParam attackParam) {
        ...
        for (List<Mutation> mList : MUTATIONS) {
            ...
            // which one
            if (attackParam.getAttack().contains(String.valueOf(mList.get(0).original))) {
                HtmlContextAnalyser hca = new HtmlContextAnalyser(attackParam.getMsg());
                // Remove all of the chrs to be mutated
                String filteredEvidence = attackParam.getAttack();
                ...
                List<HtmlContext> contexts =
                        hca.getHtmlContexts(
                                attackParam.isFindDecoded()
                                        ? getURLDecode(filteredEvidence)
                                        : filteredEvidence,
                                attackParam.getTargetContext(),
                                attackParam.getIgnoreFlags(),
                                attackParam.isIgnoreSafeParents());
                if (!contexts.isEmpty()) {
                    // Try again with new attack
                    String mutatedAttack = attackParam.getAttack();
                    String mutatedEvidence = attackParam.getAttack();
                    for (Mutation mutation : mList) {
                        mutatedAttack =
                    ...
                    attackParam.setAttack(mutatedAttack);
                    attackParam.setEvidence(mutatedEvidence);
                    return this.performAttack(attackParam);
                }
            }
        }
        return List.of();
    ...
    private boolean performDirectAttack(HttpMessage msg, String param, String value) {
        for (String scriptAlert : GENERIC_SCRIPT_ALERT_LIST) {
            List<HtmlContext> contexts2 =
                    performAttack(new XssAttackParam(msg, param, "'\"" + scriptAlert, null, 0));
            if (contexts2 == null) {
                continue;
            }
    ...
    private boolean performTagAttack(
        ...
        if (context.isInScriptAttribute()) {
            ...
            List<HtmlContext> contexts2 =
                    performAttack(new XssAttackParam(msg, param, ";alert(1)", context, 0));
            if (contexts2 == null) {
                return false;
            }
            ...
            List<HtmlContext> contexts2 =
                    performAttack(
                            new XssAttackParam(msg, param, "javascript:alert(1);", context, 0));
            if (contexts2 == null) {
                return false;
            }
    ...
    private boolean performBodyAttack(HtmlContext context, HttpMessage msg, String param) {
        ...
        for (String scriptAlert : GENERIC_SCRIPT_ALERT_LIST) {
            List<HtmlContext> contexts2 =
                    performAttack(
                            new XssAttackParam(
                                    msg, param, scriptAlert, null, HtmlContext.IGNORE_PARENT));
            if (contexts2 == null) {
                continue;
            }
            if (!contexts2.isEmpty()) {
        ...
        List<HtmlContext> contexts2 =
                performAttack(
                        new XssAttackParam(
                                msg, param, B_MOUSE_ALERT, context, HtmlContext.IGNORE_PARENT));
        if (contexts2 != null) {
            for (HtmlContext context2 : contexts2) {
    ...
    private boolean performAttributeAttack(HtmlContext context, HttpMessage msg, String param) {
        List<HtmlContext> context2 =
                performAttack(
                        new XssAttackParam(msg, param, ACCESSKEY_ATTRIBUTE_ALERT, context, 0));
        if (context2 == null) {
            return false;
        }
    ...
    private boolean performElementAttack(HtmlContext context, HttpMessage msg, String param) {
        ...
        List<HtmlContext> context2 =
                performAttack(
                        new XssAttackParam(
                                msg, param, attackString1, context, HtmlContext.IGNORE_PARENT));
        if (context2 == null) {
            context2 = performAttack(new XssAttackParam(msg, param, TAG_ONCLICK_ALERT, context, 0));
            if (context2 == null) {
                return false;
            }
            for (HtmlContext ctx : context2) {
    ...
    @AllArgsConstructor
    private class XssAttackParam {

        public XssAttackParam(
                HttpMessage msg,
                String param,
                String attack,
        ...
        @Getter @Setter private HttpMessage msg;
        @Getter private String param;
        @Getter @Setter private String attack;
        @Getter private HtmlContext targetContext;
        @Getter @Setter private String evidence;
        @Getter private int ignoreFlags;
        @Getter private boolean findDecoded;
        @Getter private boolean nullByteSpecialHandling;
        @Getter private boolean ignoreSafeParents;
        @Getter private boolean mutateAttack;
    }
}

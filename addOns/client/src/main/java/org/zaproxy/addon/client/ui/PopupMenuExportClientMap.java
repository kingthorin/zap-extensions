...
public class PopupMenuExportClientMap extends ExtensionPopupMenuItem {
    ...
    protected void performAction() {
        ...
        //                getOutputList(extension.getClientTree().getRoot());
        writeClientMap(file);
        Stats.incCounter(STATS_EXPORT_CLIENTMAP);
    }

    //    protected List<String> getOutputList(ClientNode startingPoint) {
    //        List<String> outputList = new ArrayList<>();
    //        Enumeration<?> en = (startingPoint.preorderEnumeration());
    //        while (en.hasMoreElements()) {
    //            ClientNode node = (ClientNode) en.nextElement();
    //            if (node.isRoot()) {
    //                continue;
    //            }
    //            outputList.add(node.getUserObject().getUrl());
    //        }
    //        return outputList;
    //    }
    ...
    class ClientMapWriter {
        ...
        private static void outputNode(BufferedWriter fw, ClientNode node, int level)
                throws IOException {
            // We could create a set of data structures and use snakeyaml, but the format is very
            // simple

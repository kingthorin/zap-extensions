...
public class MorseEncoder implements EncodeDecodeProcessor {
    ...
    private static final Map<Character, String> CHARACTER_MAP = Map.ofEntries(
            Map.entry('A', ".-"),
            Map.entry('B', "-..."),
            Map.entry('C', "-.-."),
            Map.entry('D', "-.."),
    ...
    public EncodeDecodeResult process(String value) {
        if (!VALID_CHARS.matcher(value.toUpperCase(Locale.ROOT)).matches()) {
            return EncodeDecodeResult.withError(Constant.messages.getString("encoder.morse.error"));
        }

        String out = "";
        for (Character c : value.toCharArray()) {
            out += CHARACTER_MAP.get(c);
        }
        return new EncodeDecodeResult(out);
    }

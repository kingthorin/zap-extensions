...
public final class ScriptRunRecorder {
    ...
    private static final int MESSAGE_MAX_LENGTH = 8388608;
                ...
                for (RunScript runScript : scripts) {
                    ...
                    for (RunStep step : runScript.steps()) {
                        ...
                        for (StepOutput output : step.outputs()) {
                            ...
                            out.setMessage(
                                    truncateMessage(StringUtils.defaultString(output.message())));
    ...
    private static String truncateMessage(String message) {
        ...
        int keep = MESSAGE_MAX_LENGTH - TRUNCATED_SUFFIX.length();
        if (keep < 0) {
            return TRUNCATED_SUFFIX.substring(0, MESSAGE_MAX_LENGTH);
        }
        return message.substring(0, keep) + TRUNCATED_SUFFIX;
    }
}

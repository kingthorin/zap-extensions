 ...
 *
 * <p>This is used by the script automation framework to enable script chaining where multiple
 * scripts need to share browser sessions and WebDriver instances.
 *
 * @since 48.12.0
 */
public final class ZestScriptMerger {

    private static final Logger LOGGER = LogManager.getLogger(ZestScriptMerger.class);
     ...
     * @throws IllegalArgumentException if scripts list is null or empty
     */
    public static ZestScriptWrapper mergeScripts(
            List<ZestScriptWrapper> scripts, String mergedScriptName) {
        ...
        for (int scriptIndex = 0; scriptIndex < scripts.size(); scriptIndex++) {
            ...
            int disabledLaunchCount = (scriptIndex == 0) ? 0 : countClientLaunches(script);
        ...
        if (countClientLaunches(firstZestScript) > 0 && !windowHandles.isEmpty()) {
        ...
        LOGGER.debug("Before serialization: {} statements", mergedScript.getStatements().size());

        // Create wrapper via JSON serialization (no setZestScript method exists)
        ExtensionZest extZest = getExtensionZest();
        if (extZest == null) {
            throw new IllegalStateException(
                    "ExtensionZest is not available - cannot merge scripts");
        }
        String mergedJson = extZest.convertElementToString(mergedScript);

        LOGGER.debug("JSON length: {} chars", mergedJson.length());
        ...
        ZestScriptWrapper result = new ZestScriptWrapper(sw);

        LOGGER.debug(
                "After deserialization: {} statements",
                result.getZestScript().getStatements().size());
    ...
    private static void addHandleIfClientWindow(ZestStatement statement, Set<String> handles) {
        if (statement instanceof ZestClientLaunch launch) {
            String h = launch.getWindowHandle();
            if (h != null && !h.isEmpty()) {
                handles.add(h);
            }
        } else if (statement instanceof ZestClientWindowHandle windowHandle) {
            String h = windowHandle.getWindowHandle();
            if (h != null && !h.isEmpty()) {
                handles.add(h);
            }
        }
     ...
     */
    private static int countClientLaunches(ZestScript script) {
        int count = 0;
        for (ZestStatement statement : script.getStatements()) {
            if (statement instanceof ZestClientLaunch) {
                count++;
            }
        }
        return count;
    ...
    /**
     * Gets the ExtensionZest instance.
     *
     * @return The ExtensionZest instance
     */
    private static ExtensionZest getExtensionZest() {
        return Control.getSingleton().getExtensionLoader().getExtension(ExtensionZest.class);
    }
}

...
class ZestScriptMergerUnitTest {
    ...
    @Test
    void shouldRejectNullScriptList() {
        // When / Then
        IllegalArgumentException exception =
                assertThrows(
                        IllegalArgumentException.class,
                        () -> ZestScriptMerger.mergeScripts(null, "merged"));

        assertThat(exception.getMessage(), containsString("must not be null or empty"));
    }

    @Test
    void shouldRejectEmptyScriptList() {
        // When / Then
        IllegalArgumentException exception =
                assertThrows(
                        IllegalArgumentException.class,
                        () -> ZestScriptMerger.mergeScripts(List.of(), "merged"));

        assertThat(exception.getMessage(), containsString("must not be null or empty"));
    }

    @Test
    void shouldThrowWhenExtensionZestNotAvailable() {
        // Given
        ZestScriptWrapper script1 = createMockZestWrapper("script1", 2);

        var extLoader = mock(ExtensionLoader.class);
        Control.initSingletonForTesting(mock(Model.class), extLoader);
        given(extLoader.getExtension(ExtensionZest.class)).willReturn(null);

        // When / Then
        IllegalStateException exception =
                assertThrows(
                        IllegalStateException.class,
                        () -> ZestScriptMerger.mergeScripts(List.of(script1), "merged"));

        assertThat(exception.getMessage(), containsString("ExtensionZest is not available"));
    }
    ...
    void shouldMergeTwoScripts() {
        ...
        ZestScriptWrapper merged =
                ZestScriptMerger.mergeScripts(List.of(script1, script2), "merged");
    ...
    void shouldMergeThreeScripts() {
        ...
        // When
        ZestScriptWrapper merged =
                ZestScriptMerger.mergeScripts(List.of(script1, script2, script3), "merged_chain");
    ...
    void shouldHandleSingleScript() {
        // Given
        ZestScriptWrapper script1 = createMockZestWrapper("script1", 5);

        // When
        ZestScriptWrapper merged = ZestScriptMerger.mergeScripts(List.of(script1), "single");
    ...
    void shouldHandleScriptWithNoStatements() {
        // Given
        ZestScriptWrapper script1 = createMockZestWrapper("empty", 0);
    ...
    void shouldDisableClientLaunchInThirdScript() {
        ...
        // When
        ZestScriptWrapper merged =
                ZestScriptMerger.mergeScripts(List.of(script1, script2, script3), "merged");
    ...
    void shouldPreserveFirstScriptMetadata() {
        // Given
        ZestScriptWrapper script1 = createMockZestWrapper("first_script", 2);
    ...
    void shouldDeepCopyStatements() {
        ...
        // When
        ZestScriptWrapper merged = ZestScriptMerger.mergeScripts(List.of(script1), "merged");
    ...
    void shouldCloseAllWindowHandlesIncludingThoseFromWindowHandleStatements() {
        ...
        // When
        ZestScriptWrapper merged =
                ZestScriptMerger.mergeScripts(List.of(wrapper1, script2), "merged");
    ...
    @Test
    void shouldNotAddBrowserCloseWhenFirstScriptHasNoClientLaunch() {
        // Given
        ZestScriptWrapper script1 = createMockZestWrapper("script1", 2);
        ZestScriptWrapper script2 = createMockZestWrapper("script2", 1);
        ...
        // Then
        for (ZestStatement stmt : merged.getZestScript().getStatements()) {
            if (stmt instanceof ZestComment
                    && ((ZestComment) stmt).getComment().contains("=== END:")) {
                throw new AssertionError("Unexpected END comment when first script has no launch");
            }
            if (stmt instanceof ZestClientWindowClose) {
                throw new AssertionError(
                        "Unexpected ZestClientWindowClose when first script has no launch");
            }
        }
    }
